...
//         timestamp: 100000000 - (i ? i * gap : 1),
//         message: getRandomLog(),
//       }));
//       console.log(delay, limit, endTs, randomPastTs, gap);
//       setTimeout(() => {
...
//   const dataValues = [];
//   for (let i = 0; i <= 100000000; i += stepSize * 1000) {
//     dataValues.push({
//       timestamp: i,
...
const populateDataValues = (startTs, endTs, stepSize, min = 0, max = 100) => {
  const dataValues = [];
  for (let i = 100; i <= 10000; i += 1 * 1000) {
    dataValues.push({
      timestamp: i,
      value: min + Math.floor(Math.random() * (max - min) * 100) / 100,
    });
...
export class MimicMetrics {
  static fetchMetrics({ startTs, endTs }) {
    ...
    const getGraph = (name, linesConfig) => ({
      ...
      graphLines: linesConfig.map(([name, min, max]) => ({
        name,
        values: populateDataValues(startTs, endTs, 1, min, max),
      })),
    });
    const getProportionateMinMax = (min, max, newPeak) => [
      (min * newPeak) / 100,
